...
data = pd.read_csv("D:/4th-Year/Research Proposal/Dataset/positioning dataset/beacon_readings.csv")
...
for idx in np.unique(y):
    ...
    mean_scores[int(idx)] = np.mean(np.array(temp_score))
    std_scores[int(idx)] = np.std(np.array(temp_score)) 
